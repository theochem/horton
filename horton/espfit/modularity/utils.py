try:
    ...
    print("Warning, using built-in utils")
    def typecheck_geo(coordinates=None, numbers=None, pseudo_numbers=None,
        ...
        if coordinates is None:
            ...
            if coordinates.shape != (natom, 3) or not issubclass(coordinates.dtype.type,
                                                                 float):
                raise TypeError(
        ...
        if numbers is None:
            ...
            if numbers.shape != (natom,) or not issubclass(numbers.dtype.type, int):
                raise TypeError(
                    'The argument numbers must be a vector with length natom.')
