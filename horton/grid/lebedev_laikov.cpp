...
/* lebedev_laikov_npoint
   ...
   lvalue
      grid complete through this value of angular momentum quantum number l.

   return value
      number of points in sought Lebedev-Laikov grid.
*/
int lebedev_laikov_npoint(int lvalue)
{
  ...
  if (lvalue < 0) {
    throw std::domain_error("lvalue can not be negative.");
  } else if (lvalue < 3) {
    return 6;
  } else if (lvalue <= 31) {
  ...
  throw std::domain_error("Unsuported angular momentum in lebedev_laikov_npoint.");
}
