...
def test_go_basis_desc_hydrogen_321g():
    ...
    assert (system.obasis.nprims == np.array([2,1])).all()
    assert (system.obasis.shell_types == np.array([0,0])).all()
    assert abs(system.obasis.alphas - np.array([5.4471780, 0.8245470, 0.1831920])).all() < 1e-10
    assert abs(system.obasis.con_coeffs - np.array([0.1562850, 0.9046910, 1.0000000])).all() < 1e-10
    assert system.obasis.nbasis == 2
...
def test_go_basis_desc_lithium_321g():
    ...
    assert (system.obasis.shell_map == np.array([0,0,0,0,0])).all()
    assert (system.obasis.nprims == np.array([3,2,2,1,1])).all()
    assert (system.obasis.shell_types == np.array([0,0,1,0,1])).all()
    assert abs(system.obasis.alphas - np.array([
        36.8382000, 5.4817200, 1.1132700,
        0.5402050, 0.1022550, 0.5402050, 0.1022550,
        0.0285650, 0.0285650,
    ])).all() < 1e-10
    assert abs(system.obasis.con_coeffs - np.array([
        0.0696686, 0.3813460, 0.6817020,
        -0.2631270, 0.1615460, 1.1433900, 0.9156630,
        1.0000000, 1.0000000
    ])).all() < 1e-10
    assert system.obasis.nbasis == 9


def test_go_basis_desc_water_sto3g():
    fn = context.get_fn('test/water_element.xyz')
    system = System.from_file(fn, obasis='STO-3G')
    assert (system.obasis.shell_map == np.array([0,1,1,1,2])).all()
    assert (system.obasis.nprims == np.array([3,3,3,3,3])).all()
    assert (system.obasis.shell_types == np.array([0,0,0,1,0])).all()
    assert abs(system.obasis.alphas - np.array([
