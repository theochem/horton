...
def test_base_exceptions():
    ...
    grid = BeckeMolGrid(sys.coordinates, sys.numbers, sys.pseudo_numbers, (rgrid, 110), random_rotate=False, mode='discard')
    with assert_raises(ValueError):
        # the default setting is local=true, which is not compatible with mode='discard'.
        dp = WPart(sys, grid)

    grid = BeckeMolGrid(sys.coordinates, sys.numbers, sys.pseudo_numbers, (rgrid, 110), random_rotate=False, mode='keep')
...
def test_wpart_schemes():
    ...
    assert wpart_schemes['hi'].options == ['slow', 'lmax', 'threshold', 'maxiter', 'greedy', 'epsilon']
