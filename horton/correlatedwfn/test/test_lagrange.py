...
def test_ap1rog_lagrange():
    ...
    energy, g, l = geminal_solver(one, er, external['nn'], exp_alpha, olp, True, **{'solver': {'wfn': 'hybr', 'lagrange': 'hybr'}, 'checkpoint': -1, 'maxiter': {'orbiter': 50}})

    geminal_solver.lagrange.assign_array(np.random.rand(3,25))
    x = geminal_solver.geminal._array.ravel(order='C')
    dxs = np.random.rand(20, 3*25)*(0.00001)
...
def fun(x, ham):
    ...
    lagrangian = ham.get_total_energy(x.reshape(3,25))
    lagrangian += np.dot(coeff.ravel(order='C'), ham.vector_function_ap1rog(x.ravel(order='C'),kmatrix,vmatrix,vmatrixa,one_mo,energy))
    return lagrangian
...
def fun_deriv(x, ham):
    ...
    gradient = ham.vector_function_lambda(coeff,x.reshape(3,25),kmatrix,vmatrix,vmatrixa,one_mo,energy)
