...
The builtin Horton scripts all have the following filename pattern: ``horton-*.py``. Through command line arguments, one can control the actual calculation. Basic usage information is obtained with the ``--help`` flag, e.g.:
...
All input and output of data in Horton is done through the :py:class:`~horton.io.iodata.IOData` class. Data can be loaded with a call to the class method :py:meth:`~horton.io.iodata.IOData.from_file` of the :py:class:`~horton.io.iodata.IOData` class, e.g.:
...
Writing data to a file is done by first creating a :py:class:`~horton.io.iodata.IOData` instance, followed by setting the appropriate attributes and finally followed by a call to the :py:meth:`~horton.io.iodata.IOData.to_file` method of the :py:class:`~horton.io.iodata.IOData` class, e.g. the following creates a ``.xyz`` file with a Neon atom:
...
A Complete list of supported file formats for data input or output can be found here: :ref:`ref_file_formats`, including a list of :py:class:`~horton.io.iodata.IOData` attributes supported by each file format. A definition of all possible :py:class:`~horton.io.iodata.IOData` attributes can be found here: :py:class:`horton.io.iodata.IOData`.
