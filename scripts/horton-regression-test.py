...
* rt_thresholds: the allowed maximum absolute deviation from the reference result.
...
by more than the threshold. It also prints unit test source code that can be included in
HORTON.
...
'''.format(comment_regtest[:-1], default_threshold, comment_auto[:-1])
...
def main():
    ...
    for example_path in example_paths:
        ...
        rt_thresholds = example_globals.get('rt_thresholds', {})
        rt_references = example_globals.get('rt_references', {})
        rt_previous = example_globals.get('rt_previous', {})
        known_keys = ['rt_results', 'rt_thresholds', 'rt_references', 'rt_previous']
        ...
        if do_update:
            ...
            exit_status |= report_regressions(rt_results, rt_thresholds, rt_references,
